...
def get_coordinates(location):
    ...
    dict = {}
    for i in location:
        driver.find_element_by_id('txtSearch').clear()      # 清空输入框
        time.sleep(0.1)
        driver.find_element_by_id('txtSearch').send_keys(i)   # 输入框内写入内容
        driver.find_element_by_css_selector('#txtSearch+a').click()        # 点击搜索
        time.sleep(0.1)
        coordinates = driver.find_element_by_css_selector('#txtCoordinate').get_attribute('value')       # 复制坐标
